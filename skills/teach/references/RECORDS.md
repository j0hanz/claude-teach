...
`doubling` defaults to 2, `ceiling` to 90; override both via `spacing: { doubling: N, ceiling: N }` under `NOTES.md` `## Preferences` — exact shape only, any other shape does not parse and built-in 2/90 stands.
...
When later record contradicts earlier one (understanding deepened or corrected), mark old record `status: superseded by NNNN-slug` instead of deleting. History of how understanding evolved itself useful signal.
