...
Token values live in [`templates/assets/styles.css`](../../../templates/assets/styles.css) `:root` — the single source. Edit there; never restate values here.
...
The swap text comes from `data-unsealed-label` on `.seal-note`, so it translates with the lesson, and it carries the one instruction the page otherwise never gives: **paste the result line back**. Without it the cold open ends in a mono box reading `Cold open 0007-x: 1 right, 2 wrong` beside a Copy button, and nothing anywhere on the page says what either is for — the spaced-repetition loop depends on a learner action the lesson never asks for. The slot costs nothing: the release announcement had to happen anyway.
