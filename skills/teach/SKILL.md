...
- Step 8 asked and nothing came back: increment `asked:` in the line. At `asked: 2` the line is abandoned — `teach.py score "abandon"` reschedules each record it tested (no credit, no lapse) and deletes the line. Result is unrecoverable; leaving the line in place blocks every future cold open.
