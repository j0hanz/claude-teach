...
Two kinds of strength, and most study tools build the wrong one. **Fluency** feels like mastery in the moment and fades. **Storage** is what you still have next month. Claude Teach builds storage through desirable difficulty: retrieval practice (recall from memory, before each lesson), spacing (practice distributed across sessions), and interleaving (related topics mixed, for skills).
...
```text
/plugin marketplace add j0hanz/claude-teach
/plugin install claude-teach@claude-teach
```

Or clone and install from a local path:

```bash
git clone https://github.com/j0hanz/claude-teach.git
```

```text
/plugin marketplace add /absolute/path/to/claude-teach
/plugin install claude-teach@claude-teach
```
